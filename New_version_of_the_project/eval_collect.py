...
def parse_args():
    ...
    # === CHANGE: Path to your data.yaml file ===
    ap.add_argument("--data", default="data.yaml", help="Path to data.yaml")

    # === CHANGE: Path to runs directory (can be single run or parent with multiple runs) ===
    # Examples: "runs" (evaluates all), "runs/baseline_s0" (evaluates one)
    ap.add_argument("--runs", default="runs", help="A run dir OR a parent dir containing multiple runs")

    # === CHANGE: Output CSV filename where metrics will be saved ===
    ap.add_argument("--out", default="metrics_summary.csv", help="Output CSV")

    # === CHANGE: Which dataset split to evaluate on ===
    ap.add_argument("--split", default="test", choices=["val", "test"], help="Which split to evaluate on")

    # === CHANGE: Set to True if you want to generate PR/F1 plots during evaluation ===
...
def main():
    ...
    for rd in runs:
        ...
        # === CHANGE: Modify this if you have different number of classes ===
